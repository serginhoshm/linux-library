...
------------------------------------------------------------
 ...
 - compartilhar os downloads entre o usuario e o daemon
...
install_packages() {
    ...
    if [[ -f /etc/debian_version ]]; then
        ...
        echo "Distribuicao nao suportada automaticamente. Requer Linux Mint/Ubuntu/Debian ou Fedora/RHEL-like."
        exit 1
    fi
}
...
prepare_download_dir() {
    echo "[3/7] Preparando pasta de downloads para o usuario e o daemon..."
    mkdir -p "${DOWNLOAD_DIR}"
    chown -R "${REAL_USER}:${TRANSMISSION_GROUP}" "${DOWNLOAD_DIR}"
    chmod -R u+rwX,g+rwX,o-rwx "${DOWNLOAD_DIR}"
    find "${DOWNLOAD_DIR}" -type d -exec chmod g+s {} +
}

detect_service() {
    echo "[2/7] Detectando servico e usuario do Transmission..."
...
------------------------------------------------------------
 ...
 Permissoes: usuario e grupo do Transmission; umask 007
 Cliente CLI: transmission-remote 127.0.0.1:9091 -l
 Web UI/RPC: http://${host_ref}:9091
 Configuracao: ${SETTINGS_FILE}

 O RPC esta sem senha, restrito a enderecos locais pela whitelist.
 Use apenas em uma LAN domestica e mantenha o firewall ativo.
============================================================
EOF
}
